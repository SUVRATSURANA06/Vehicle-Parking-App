...
@admin_bp.route('/reports/export-monthly-csv', methods=['POST'])
@login_required
@admin_required
def export_monthly_csv():
    ...
    try:
        ...
        use_background_task = data.get('background', False)
        
        if use_background_task:
            # Use background task for large datasets
            from tasks import export_monthly_bookings_csv
            task = export_monthly_bookings_csv.delay()
            return jsonify({
                'success': True,
                'message': 'Monthly CSV export started in background',
                'task_id': task.id,
                'background': True
            })
        ...
        # Prepare data for CSV with detailed booking information
        data = []
        for reservation in reservations:
            ...
            # Convert UTC timestamps to local timezone (IST - UTC+5:30)
        ...
        return jsonify({
            'success': True,
            'message': f'Monthly booking data exported successfully with {len(data)} records.',
            'filename': csv_filename,
            'download_url': f'/admin/reports/download-csv/{csv_filename}'
        })
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)})

@admin_bp.route('/reports/download-csv/<filename>')
@login_required
@admin_required
def download_monthly_csv(filename):
...
@admin_bp.route('/reports/csv-status/<task_id>')
@login_required
@admin_required
def csv_export_status(task_id):
    ...
    task = export_monthly_bookings_csv.AsyncResult(task_id)
    if task.ready():
        result = task.result
        if 'completed' in result:
            ...
            return jsonify({
                'status': 'completed', 
                'filename': filename,
                'download_url': f'/admin/reports/download-csv/{filename}'
            })
        else:
